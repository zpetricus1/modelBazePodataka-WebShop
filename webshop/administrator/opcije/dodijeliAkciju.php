<?php
  ...
  require_once '../../skripteBP/dodijelaAkcijeProizvodu.php';
  ...
  <div class="body">
    ...
    <div class="bodyContent">
      ...
      <form class="" action="?" method="post">
        <label for="proizvod_id">Odaberite ID proizvoda: </label>
        <select name ="proizvod_id" id="proizvod_id">
        <?php
          $con=pg_connect("host=localhost dbname=webshop user=postgres password=182ozux33");
          $res=pg_query($con,"select proizvod_id from proizvodi");
          while($row=pg_fetch_array($res)){
         ?>
         <option><?php echo $row["proizvod_id"]; ?></option><br><br><br>
         <?php
         }
          ?>
          <select name ="proizvod_id" id="proizvod_id">
          ...
          <label for="akcija">Odaberite akciju: </label>
          <select name ="akcija" id="akcija">
          <?php

            $res=pg_query($con,"select * from akcija");
            while($row=pg_fetch_array($res)){
           ?>
           <option><?php echo $row["iznos"]; ?></option><br><br><br>
           <?php
           }
            ?>
        ...
        <input type="submit"name="submit" id="submit" class="buttonPrijava" value="Dodijeli"><br><br>
      </form>
